...
%    vols_wt_b_coeff_clean = sqrt(n) * src_vols_wt_backward(src, basis, wts, vols_wt_est_opt);
%    noise_wt_b_coeff = sqrt(n) * src_vols_wt_backward(noise_src, basis, wts, vols_wt_est_opt);
%    disp(['Finished with clean backproj!, t = ' num2str(toc)]);

%    for noise_idx = 1:length(noise_levels)
%        vols_wt_b_coeff = vols_wt_b_coeff_clean + ...
%            sqrt(noise_levels(noise_idx)) * noise_wt_b_coeff;
%        disp(['Finished with noise backproj! Noise level = ' ...
%            num2str(noise_levels(noise_idx)) ', t = ' num2str(toc)]);
