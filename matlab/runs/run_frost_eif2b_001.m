root_dir = fullfile(pkg_root(), 'data', 'frost_eif2b');

star_file = fullfile(root_dir, 'AUG11_99Kparticles_updatedGCTF.star');

star_opt = struct();
star_opt.pixel_size = 0.838;
...
src_cropped = crop_src(src_orig, [79 79], [378 378]);
...
src_aug.L = src_ds.L;
src_aug.n = 2*src_ds.n;
src_aug.precision = src_ds.precision;
src_aug.images = repmat(src_ds.images, [1 1 2]);
src_aug.params = struct();
src_aug.params.rots = cat(3, src_ds.params.rots, sym_rots);
src_aug.params.filters = src_ds.params.filters;
src_aug.params.filter_idx = repmat(src_ds.params.filter_idx, [1 2]);
src_aug.params.offsets = repmat(src_ds.params.offsets, [1 2]);
src_aug.params.amplitudes = repmat(src_ds.params.amplitudes, [1 2]);
fprintf('OK (%.2f s)\n', toc(tmr));
